...
async def main_task():
    ...
    # --- MODIFIKASI DIMULAI ---
    # Hilangkan pemilihan token interaktif.
    # Hardcode token dan amount sesuai permintaan.
    
    token_a_symbol = "ZTUSD"
    token_b_symbol = "MOLLY"
    amount_a_str = "0.05"  # Hardcoded 0.05 ZTUSD
    amount_b_str = "8.282" # Hardcoded 8.282 MOLLY
    
    # --- MODIFIKASI BARU: Hardcode jumlah pengulangan ---
    repeats = 25 
    # --- ------------------------------------------ ---

    try:
        # Cari objek token dari list yang di-fetch
        token_a = next((t for t in tokens if t['symbol'] == token_a_symbol), None)
        token_b = next((t for t in tokens if t['symbol'] == token_b_symbol), None)

        if not token_a or not token_b:
            logger('error', f'Token {token_a_symbol} atau {token_b_symbol} tidak ditemukan di subgraph.')
            return
        
        logger('info', f'Swap dikonfigurasi: {amount_a_str} {token_a["symbol"]} -> {token_b["symbol"]}')
        logger('info', f'Swap kembali dikonfigurasi: {amount_b_str} {token_b["symbol"]} -> {token_a["symbol"]}')
        
        # --- MODIFIKASI: Hapus input jumlah pengulangan ---
        # repeat_str = input(f'\n{Fore.YELLOW}Berapa kali swap per wallet? (default: 1): {Style.RESET_ALL}')
        # repeats = int(repeat_str) if repeat_str.isdigit() and int(repeat_str) > 0 else 1
        logger('info', f'Setiap wallet akan menjalankan {repeats} siklus swap.')
        # --- ------------------------------------------ ---
    ...
    for pk in pks:
        ...
        try:
            for j in range(repeats):
                ...
                # Gunakan token A, B, dan amount A yang sudah di-hardcode
                ...
                if swap_success:
                    ...
                    # Gunakan amount B yang sudah di-hardcode
                    amount_b_to_swap_str = amount_b_str 
